...
def _extract_sections(full_resume_text, persist_directory): 
    ...
    for section in sections:
        ...
        sectionDict[section] = ""
        query = "Begin all returned text with: \
            'This is a section of a resume.' \
            Return the heading " + section + \
                " and the text under the " + section + "section of the resume."
        docs = docsearch.get_relevant_documents(query)
    ...
    # This is a base VectorStore, rather than VectorStoreRetriever    
    docsearch2 = Chroma.from_texts(list(sectionDict.values()), 
                                   embeddings, persist_directory=persist_directory,
                                   metadatas=[{"source": str(i)} for i in range(len(list(sectionDict.values())))])
    docsearch2.persist()
